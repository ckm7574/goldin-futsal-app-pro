...
type Session = {
  ...
  defAwards: Record<TeamId, string[]>;
...
function normalizeLoaded(data: any): PersistShape {
  ...
  if (data?.sessionsByDate && typeof data.sessionsByDate === "object") {
    for (const [k, v] of Object.entries<any>(data.sessionsByDate)) {
      ...
      const parseDef = (v: any): string[] => {
  // Backward compatible:
  // - old format: string | null
  // - new format: string[] (max 2)
  if (Array.isArray(v)) return v.filter((x: any) => typeof x === "string" && x).slice(0, 2);
  if (typeof v === "string" && v) return [v];
  return [];
};
const defAwards: Record<TeamId, string[]> = {
  A: parseDef(s?.defAwards?.A),
  B: parseDef(s?.defAwards?.B),
  C: parseDef(s?.defAwards?.C),
  D: parseDef(s?.defAwards?.D),
};
...
const teamNames = (s?.teamNames && typeof s.teamNames === "object")
  ...
  Object.values(sessionsByDate).forEach(sess => {
    ...
    // defAwards: normalize legacy (string|null) -> string[] (max 2)
    if (!(sess as any).defAwards) (sess as any).defAwards = { A: [], B: [], C: [], D: [] };
    (["A","B","C","D"] as TeamId[]).forEach(tid => {
      const v: any = (sess as any).defAwards?.[tid];
      if (Array.isArray(v)) (sess as any).defAwards[tid] = v.filter((x: any) => typeof x === "string" && x).slice(0, 2);
      else if (typeof v === "string" && v) (sess as any).defAwards[tid] = [v];
      else (sess as any).defAwards[tid] = [];
    });
    if ((sess as any).hasTeamD && !(sess as any).rosters?.D) (sess as any).rosters.D = [];
...
const patchSession = (patch: Partial<Session>) => {
  ...
  const setDef = (tid: TeamId, pids: string[]) => patchSession({ defAwards: { ...(cur.defAwards || { A: [], B: [], C: [], D: [] }), [tid]: (pids || []).filter(Boolean).slice(0, 2) } });
  ...
  function calcScores(session: Session) {
    ...
    const teamBonusByTeam = computeTeamBonus(standings.filter(r => getActiveTeamsSafe(session).includes(r.team)), (session as any)?.hasTeamD || false);
    ...
    Object.keys(out).forEach(pid => {
      ...
      const picks = team !== "-" ? asArray<string>(session.defAwards?.[team], []) : [];
      const def = team !== "-" && picks.includes(pid) ? (picks.length === 1 ? 2 : 1) : 0;
  ...
  function top5RankingFrom(baseList: any[], type: "goals" | "assists" | "def" | "cleansheets" | "teamBonus" | "total") {
    ...
    if (type === "cleansheets") {
      // ✅ 세션별 GK/필드 전환이 있을 수 있으므로, 누적 CS가 1 이상인 선수만 표시
      sorted = sorted.filter(p => asNumber(p.cleansheets, 0) > 0);
    }
            ...
            {!(typeof cur !== "undefined" ? (cur as any)?.hasTeamD : false) && (
              <button className="btn add-team" onClick={() => {
                setState(prev => {
                  ...
                  base.defAwards = { ...base.defAwards, D: (Array.isArray((base as any).defAwards?.D) ? (base as any).defAwards.D : []) };
  ...
  <label style={{ marginRight: 8 }}>🛡 수비상</label>
  {(() => {
    const arr = asArray<string>(cur.defAwards?.[tid], []);
    const d1 = arr[0] || "";
    const d2 = arr[1] || "";
    const rosterIds = asArray(cur.rosters[tid], []);
    const options = rosterIds.map(pid => ({ pid, name: players.find(p => p.id === pid)?.name || "?" }));
    const apply = (next1: string, next2: string) => {
      const a = [next1, next2].filter(Boolean);
      const uniq: string[] = [];
      a.forEach(x => { if (x && !uniq.includes(x)) uniq.push(x); });
      setDef(tid, uniq.slice(0, 2));
    };
    return (
      <>
        <span className="hint">팀당 최대 2명 (2명 선택: 각 1점 / 1명만 선택: 2점)</span>
        <select
          value={d1}
          onChange={e => apply(e.target.value || "", d2)}
          disabled={readonly}
          style={{ minWidth: 140 }}
        >
          <option value="">(1번 선택 없음)</option>
          {options.map(o => <option key={o.pid} value={o.pid}>{o.name}</option>)}
        </select>
        <select
          value={d2}
          onChange={e => apply(d1, e.target.value || "")}
          disabled={readonly}
          style={{ minWidth: 140 }}
        >
