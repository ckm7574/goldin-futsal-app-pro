...
async function loadPersistFromSupabase(): Promise<AppPersist | null> {
  try {
    ...
    const st = (data?.state as any) ?? null;
    return st ? (normalizeLoaded(st) as any as AppPersist) : null;
...
// ====== MIGRATION: 동일 선수(동일 이름) 중복 생성 + 이름/라벨 키로 저장된 기록을 canonical id로 병합 ======
// 특히 GK→필드 전환 과정에서 같은 이름으로 다른 id가 생기거나, matchStats가 이름 기반 키로 저장된 경우를 정리한다.
const cleanKey = (v: any) => String(v ?? "")
  ...
  .replace(/[.,·ㆍ\-_/\\|:;"'`~!@#$%^&*+=?]/g, "")
  .replace(/[0-9]/g, "")
  .replace(/(골키퍼|GOALKEEPER|KEEPER|GK|필드플레이어|필드|FIELDPLAYER|FIELD|PLAYER)/gi, "")
  .replace(/[🧤🛡⚽🎯■□◆◇●○⬛⬜🟥🟦🟩🟨]/g, "")
  .trim();
...
for (const [dateKey, s] of Object.entries(sessionsByDate)) {
  const sess = s || emptySession();
  const rosters2: any = {};
  TEAM_IDS.forEach(tid => {
    rosters2[tid] = remapArr(asArray<any>(sess.rosters?.[tid], []));
  });

  const matches2 = asArray<any>(sess.matches, []).map((m: any) => {
    const gkHome = resolvePid(m?.gkHome || sess.gkHome);
    const gkAway = resolvePid(m?.gkAway || sess.gkAway);
    return { ...(m || {}), gkHome: gkHome || null, gkAway: gkAway || null };
  });

  const matchStats2: any = {};
  const ms = sess.matchStats || {};
  for (const [mid, rowAny] of Object.entries<any>(ms)) {
    const row = rowAny || {};
    const row2: any = {};
    for (const [k, v] of Object.entries<any>(row)) {
      const pid = resolvePid(k);
      if (!pid) continue;
      const cur = row2[pid] || { goals: 0, assists: 0 };
      row2[pid] = {
        goals: asNumber(cur.goals, 0) + asNumber(v?.goals, 0),
        assists: asNumber(cur.assists, 0) + asNumber(v?.assists, 0),
      };
    }
    matchStats2[mid] = row2;
  }

  const defAwards2: any = {};
  TEAM_IDS.forEach(tid => {
    const pid = resolvePid(sess.defAwards?.[tid] ?? "");
    defAwards2[tid] = pid || null;
  });

  const defAwards2v2: any = (sess.defAwards2 && typeof sess.defAwards2 === "object")
    ? { ...sess.defAwards2 }
    : { A: [null, null], B: [null, null], C: [null, null], D: [null, null] };
  TEAM_IDS.forEach(tid => {
    const arr = asArray<any>(defAwards2v2?.[tid], []);
    defAwards2v2[tid] = [resolvePid(arr[0]) || null, resolvePid(arr[1]) || null];
    if (defAwards2v2[tid][0] && defAwards2v2[tid][1] && defAwards2v2[tid][0] === defAwards2v2[tid][1]) defAwards2v2[tid][1] = null;
  });

  const posOverrides2: any = {};
  if (sess.posOverrides && typeof sess.posOverrides === "object") {
    for (const [k, v] of Object.entries<any>(sess.posOverrides)) {
      const pid = resolvePid(k);
      if (!pid) continue;
      posOverrides2[pid] = (v === "GK") ? "GK" : "필드";
    }
  }
...
// Cloud sync: authed일 때 상태를 Supabase에 업로드 (debounced)
useEffect(() => {
  if (!authed) return;
  const nextState: AppPersist = { players, teamNames: globalTeamNames, sessionsByDate, sessionDate };
  if (debTimerRef.current) clearTimeout(debTimerRef.current);
  debTimerRef.current = setTimeout(() => { void savePersistToSupabase(nextState); }, 500);
...
const isNewDefRule = Boolean(dateKey && String(dateKey) >= "2026-02-08");
const defPicksLegacy = (team !== "-" ? (session.defAwards?.[team] ?? null) : null) as any;
const defPicksV2 = (team !== "-" ? asArray<any>(session.defAwards2?.[team], []) : []) as any[];
const picks = isNewDefRule
  ? (defPicksV2.filter(Boolean).length ? defPicksV2 : (defPicksLegacy ? [defPicksLegacy] : []))
  : (defPicksLegacy ? [defPicksLegacy] : []);
const uniq = Array.from(new Set(picks.filter(Boolean))) as string[];
const def = (() => {
  if (team === "-") return 0;
  if (!uniq.length) return 0;
  if (!isNewDefRule) return uniq[0] === pid ? 2 : 0;
  // 2/8부터: 2명 선택 시 각 1점, 1명만 선택 시 2점
  if (uniq.length >= 2) return uniq.includes(pid) ? 1 : 0;
  return uniq[0] === pid ? 2 : 0;
})();
  ...
  const dailyScores = useMemo(() => calcScores(cur, sessionDate), [cur, players, globalTeamNames]);
