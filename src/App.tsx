...
const DEF_RULE_CUTOFF = "2026-02-08"; // 2/8 경기부터: 수비상 2명(각 1점), 1명이면 2점
const isNewDefRule = (dateKey: string) => String(dateKey || "") >= DEF_RULE_CUTOFF;
...
type Session = {
  ...
  defAwards2?: Record<TeamId, (string | null)[]>; // 2/8~ 팀당 최대 2명
...
function emptySession(): Session {
  return {
    ...
    defAwards2: { A: [null, null], B: [null, null], C: [null, null], D: [null, null] },
    teamNames: undefined,
    notes: "",
    hasTeamD: false,
...
function normalizeLoaded(data: any): PersistShape {
  ...
  if (data?.sessionsByDate && typeof data.sessionsByDate === "object") {
    for (const [k, v] of Object.entries<any>(data.sessionsByDate)) {
      ...
      const defAwards2: Record<TeamId, (string | null)[]> = { A: [null, null], B: [null, null], C: [null, null], D: [null, null] };
      if (s?.defAwards2 && typeof s.defAwards2 === "object") {
        (["A","B","C","D"] as TeamId[]).forEach(tid => {
          const arr = Array.isArray((s as any).defAwards2?.[tid]) ? (s as any).defAwards2[tid] : [];
          const a0 = typeof arr[0] === "string" ? String(arr[0]) : null;
          const a1 = typeof arr[1] === "string" ? String(arr[1]) : null;
          defAwards2[tid] = [a0, a1];
        });
      }
...
const teamNames = (s?.teamNames && typeof s.teamNames === "object")
      ...
      sessionsByDate[ensureSunday(k)] = { rosters, matches, matchStats, defAwards, defAwards2, teamNames, notes, rosterViewConfirmed, formations, posOverrides };
    }
  } else {
  ...
  Object.values(sessionsByDate).forEach(sess => {
    ...
    if (!sess.defAwards2) sess.defAwards2 = { A: [null, null], B: [null, null], C: [null, null], D: [null, null] } as any;
  });

  
  // ====== MIGRATION: GK->필드 전환 시 "새 선수"가 생성되어 기록/누적이 분리되는 문제 방지 ======
  // 과거 버전에서 포지션 전환을 "다른 선수(id)"처럼 저장한 경우(동일 이름의 중복 플레이어)가 있을 수 있음.
  // 여기서 이름 기준으로 중복 플레이어를 병합하고, 세션/기록의 모든 참조를 canonical id로 리매핑한다.
  const cleanKey = (v: any) => String(v ?? "")
    .normalize("NFKC")
    .replace(/[\u200B-\u200D\uFEFF]/g, "") // zero-width
    .replace(/\s+/g, "")
    .replace(/[()（）\[\]{}<>]/g, "")
    .replace(/[.,·ㆍ\-_/\\|:;"'`~!@#$%^&*+=?]/g, "")
    .replace(/(골키퍼|GOALKEEPER|KEEPER|GK|필드|FIELD|PLAYER)/gi, "")
    .replace(/[🧤🛡⚽🎯■□◆◇●○⬛⬜🟥🟦🟩🟨]/g, "")
    .trim();

  const nameToCanon = new Map<string, string>();
  const idRemap: Record<string, string> = {};
  const canonPlayers: Player[] = [];

  for (const p of players) {
    const key = cleanKey(p.name);
    if (!key) { idRemap[p.id] = p.id; canonPlayers.push(p); continue; }
    const existing = nameToCanon.get(key);
    if (!existing) {
      nameToCanon.set(key, p.id);
      idRemap[p.id] = p.id;
      canonPlayers.push(p);
    } else {
      idRemap[p.id] = existing;
      const c = canonPlayers.find(x => x.id === existing);
      if (c) {
        c.active = Boolean(c.active || p.active);
        c.pos = (c.pos === "GK" || p.pos === "GK") ? "GK" : "필드";
        // 이름은 canonical 유지 (첫 항목)
      }
    }
  }

  const canonIdSet = new Set<string>(canonPlayers.map(p => p.id));
  // strict resolver: return canonical player id if possible, otherwise "" (so we don't keep phantom ids/names in rosters/stats)
  const resolvePidLoose = (raw: any): string => {
    const s0 = String(raw ?? "");
    if (!s0) return "";
    // direct id remap
    const r0 = idRemap[s0];
    if (r0 && canonIdSet.has(r0)) return r0;
    // direct id
    if (canonIdSet.has(s0)) return s0;
    // name-ish key
    const key = cleanKey(s0);
    const pid = key ? nameToCanon.get(key) : undefined;
    return (pid && canonIdSet.has(pid)) ? pid : "";
  };
  ...
  for (const [d, s] of Object.entries(sessionsByDate)) {
    const rosters2: any = {};
    (["A","B","C","D"] as TeamId[]).forEach(tid => {
      rosters2[tid] = uniq(asArray<string>((s as any)?.rosters?.[tid], []).map(resolvePidLoose));
    });

    const matches2 = asArray<Match>((s as any).matches, []).map(m => ({
      ...m,
      gkHome: (() => { const pid = m.gkHome ? resolvePidLoose(m.gkHome) : ""; return pid || null; })(),
      gkAway: (() => { const pid = m.gkAway ? resolvePidLoose(m.gkAway) : ""; return pid || null; })(),
    }));

    const matchStats2: Record<string, MatchStats> = {};
    const rawMS2 = (s as any).matchStats || {};
    Object.keys(rawMS2).forEach(mid => {
      const row = rawMS2[mid] || {};
      const safe: MatchStats = {} as any;
      Object.keys(row).forEach(k => {
        const pid = resolvePidLoose(k);
        if (!pid) return;
        const v = row[k] || {};
        const curv = safe[pid] || { goals: 0, assists: 0, cleansheets: 0 };
        safe[pid] = {
          goals: curv.goals + asNumber(v.goals, 0),
          assists: curv.assists + asNumber(v.assists, 0),
          cleansheets: curv.cleansheets + asNumber(v.cleansheets, 0),
        };
      });
      matchStats2[mid] = safe;
    });

    const defAwards2: any = { ...(s as any).defAwards };
    (["A","B","C","D"] as TeamId[]).forEach(tid => {
      const v = (s as any).defAwards?.[tid];
      defAwards2[tid] = (() => { const pid = v ? resolvePidLoose(v) : ""; return pid || null; })();
    });

    const defAwards2v2: any = { ...(s as any).defAwards2 };
    if ((s as any).defAwards2) {
      (["A","B","C","D"] as TeamId[]).forEach(tid => {
        const arr = Array.isArray((s as any).defAwards2?.[tid]) ? (s as any).defAwards2[tid] : [];
        defAwards2v2[tid] = [arr[0] ? (resolvePidLoose(arr[0]) || null) : null, arr[1] ? (resolvePidLoose(arr[1]) || null) : null];
      });
    }

    const posOverrides2: any = {};
    if ((s as any).posOverrides && typeof (s as any).posOverrides === "object") {
      Object.entries<any>((s as any).posOverrides).forEach(([k, pv]) => {
        const pid = resolvePidLoose(k);
        if (!pid) return;
        if (pv === "GK" || pv === "필드") posOverrides2[pid] = pv;
      });
    }
    ...
    nextSessions[d] = {
      ...
      defAwards: defAwards2,
      defAwards2: defAwards2v2,
      posOverrides: posOverrides2,
    } as Session;
  }
  sessionsByDate = nextSessions;
  players = canonPlayers;
...
const patchSession = (patch: Partial<Session>) => {
  ...
  const setDef = (tid: TeamId, pid: string | null) => patchSession({ defAwards: { ...(cur.defAwards || { A: null, B: null, C: null, D: null }), [tid]: pid } });
  const setDef2 = (tid: TeamId, idx: 0 | 1, pid: string | null) => {
    const base = { ...(cur.defAwards2 || { A: [null, null], B: [null, null], C: [null, null], D: [null, null] }) } as any;
    const arr = Array.isArray(base[tid]) ? [...base[tid]] : [null, null];
    arr[idx] = pid;
    // prevent duplicates: if both same, clear second
    if (arr[0] && arr[1] && arr[0] === arr[1]) arr[1] = null;
    base[tid] = arr;
    patchSession({ defAwards2: base });
  };
  ...
  function calcScores(session: Session, dateKey?: string) {
    ...
    const normalizeKey = (k: string) => {
      let s = String(k || "").trim();
      // remove trailing "(...)" and common role labels / emojis
      s = s.replace(/\s*\(.*?\)\s*$/g, "").trim();
      s = s.replace(/(골키퍼|GK|GOALKEEPER|필드|FIELD)/gi, "").trim();
      s = s.replace(/[🧤🧱🛡⚽🎯■□]+/g, "").trim();
      s = s.replace(/\s+/g, " ").trim();
      return s;
    };
    const compact = (x: string) => normalizeKey(x).replace(/\s+/g, "");
    const resolvePid = (key: any): string | null => {
      ...
      const exact = players.find(p => normalizeKey(p.name) === nk);
      if (exact) return exact.id;

      // exact match ignoring spaces
      const nkc = nk.replace(/\s+/g, "");
      const exact2 = players.find(p => compact(p.name) === nkc);
      if (exact2) return exact2.id;

      // fuzzy contains match (handles small edits like suffix/prefix)
      const cands = players.map(p => ({ id: p.id, name: p.name, n: normalizeKey(p.name), c: compact(p.name) }));
      const hit = cands.find(p => p.n && (p.n.includes(nk) || nk.includes(p.n)));
      if (hit) return hit.id;
      const hit2 = cands.find(p => p.c && (p.c.includes(nkc) || nkc.includes(p.c)));
    ...
    const activeTeamsHere = getActiveTeams(Boolean((session as any)?.hasTeamD));
    const teamBonusByTeam = computeTeamBonus(standings.filter(r => activeTeamsHere.includes(r.team)), Boolean((session as any)?.hasTeamD));
    ...
    asArray(session.matches, []).forEach(m => {
      ...
      Object.entries(ms).forEach(([k, stat]) => {
        const pid = resolvePid(k);
        if (!pid) return;
        addStat(pid, { goals: asNumber((stat as any).goals, 0), assists: asNumber((stat as any).assists, 0) });
      });

      // Clean sheet points should always follow selected GK for that match (pid or legacy name key)
    ...
    Object.keys(out).forEach(pid => {
      ...
      const def = (() => {
        ...
        if (dk && isNewDefRule(dk)) {
          const raw = (session.defAwards2?.[team] || []) as (string | null)[];
          const uniq = Array.from(new Set(raw.filter(Boolean))) as string[];
          if (uniq.length === 0) return 0;
          if (uniq.length === 1) return uniq[0] === pid ? 2 : 0;
          return uniq.includes(pid) ? 1 : 0;
        }
        return (session.defAwards?.[team] || null) === pid ? 2 : 0;
      })();
  ...
  const dailyScores = useMemo(() => calcScores(cur, ensureSunday(sessionDate)), [cur, players, globalTeamNames, sessionDate]);
                ...
                return (
                  <div key={tid} className="team-card">
                    ...
                    <div className="row" style={{ marginTop: 8 }}>
                      ...
                      {isNewDefRule(ensureSunday(sessionDate)) ? (
                        <>
                          <select
                            value={((cur.defAwards2?.[tid]?.[0] ?? "") as any)}
                            onChange={e => setDef2(tid, 0, e.target.value || null)}
                            disabled={readonly}
                          >
                            <option value="">(1번 선택 없음)</option>
                          ...
                          <select
                            style={{ marginLeft: 8 }}
                            value={((cur.defAwards2?.[tid]?.[1] ?? "") as any)}
                            onChange={e => setDef2(tid, 1, e.target.value || null)}
                            disabled={readonly}
                          >
                            <option value="">(2번 선택 없음)</option>
                            {asArray(cur.rosters[tid], []).map(pid => {
                              const name = players.find(p => p.id === pid)?.name || "?";
                              return <option key={pid} value={pid}>{name}</option>;
                            })}
                          </select>
                          <span className="hint" style={{ marginLeft: 8 }}>팀당 2명(각 1점) / 1명만 선택 시 2점</span>
                        </>
                      ) : (
                        <select
