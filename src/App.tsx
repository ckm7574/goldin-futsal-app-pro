...
function computeTeamBonus(st: StandingRow[], hasTeamD: boolean): Record<TeamId, number> {
  ...
  // 점수 체계
  const scale = hasTeamD
    ? [4, 3, 2, 1]                  // 4팀
    : (st.length >= 3 ? [4, 2, 1]   // 3팀
                      : [4, 2]);    // 2팀

  let i = 0;
  while (i < st.length) {
    // ✅ 승점 + 골득실 동일한 공동 순위 그룹 찾기
    let j = i + 1;
    while (
      j < st.length &&
      st[j].pts === st[i].pts &&
    ...
    // 공동 순위는 동일 팀 점수 부여 (가장 높은 순위 기준 점수)
    const bonus = scale[i] ?? 0;
    for (let k = i; k < j; k++) {
      map[st[k].team] = bonus;
    }
...
const [page, setPage] = useState<1 | 2 | 3 | 4 | 5>(2);
  ...
  const toggleTheme = () => {
    const next = theme === "goldin" ? "worldcup" : "goldin";
    setTheme(next);
 ...
 // e.g., "2025-2"
  ...
  // seasonIdFromISO는 컴포넌트 밖에 선언됨 (TDZ 방지)
...
const setGkAward = (pid: string | null) => {
  ...
  function calcScores(session: Session, dateKey?: string) {
    ...
    const defAwardIdsForTeam = (team: TeamId): string[] => {
      ...
      if (Array.isArray(v)) return v.filter(Boolean).map((x: any) => resolvePid(String(x)) || String(x));
      if (typeof v === "string" && v) return [resolvePid(String(v)) || String(v)];
      return [];
    };
