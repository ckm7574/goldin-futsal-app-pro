  ...
  const hideTeamNameForGKIds = useMemo(() => {
    ...
    try {
      ...
      activeTeams.forEach((tid: TeamId) => {
        ...
        const gkIds = roster.filter(pid => (players.find(p => p.id === pid)?.pos) === "GK");
              ...
              <table className="tbl"><thead><tr>
                ...
                {sortedDaily.map((r: any, idx: number) => (
                  <tr key={r.id}>
                    <td>{idx + 1}</td>
                    <td>{r.name}</td>
                    <td>{hideTeamNameForGKIds.has(r.id) ? "" : (r.team && r.team !== "-" ? (<><span>{TEAM_LABELS[r.team as TeamId]}</span>{" "}<TeamSwatch team={r.team as TeamId} /></>) : "-")}</td>
                    <td>{r.goals || 0}</td>
                    <td>{r.assists || 0}</td>
