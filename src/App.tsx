...
const teamNames = (s?.teamNames && typeof s.teamNames === "object")
  ...
  const resolvePidLoose = (raw: any): string => {
    const s = String(raw ?? "");
    if (!s) return s;
    if (idRemap[s]) return idRemap[s];
    const key = cleanKey(s);
    const pid = nameToCanon.get(key);
    return pid || s;
  };
  ...
  for (const [d, s] of Object.entries(sessionsByDate)) {
    ...
    const matches2 = asArray<Match>((s as any).matches, []).map(m => ({
      ...m,
      gkHome: m.gkHome ? resolvePidLoose(m.gkHome) : null,
      gkAway: m.gkAway ? resolvePidLoose(m.gkAway) : null,
    }));
    ...
    (["A","B","C","D"] as TeamId[]).forEach(tid => {
      ...
      defAwards2[tid] = v ? resolvePidLoose(v) : null;
    });
    ...
    if ((s as any).defAwards2) {
      (["A","B","C","D"] as TeamId[]).forEach(tid => {
        const arr = Array.isArray((s as any).defAwards2?.[tid]) ? (s as any).defAwards2[tid] : [];
        defAwards2v2[tid] = [arr[0] ? resolvePidLoose(arr[0]) : null, arr[1] ? resolvePidLoose(arr[1]) : null];
      });
    }
