...
// ✅ 수비상 규칙 변경 기준(이 날짜부터 팀당 최대 2명/각 1점, 1명만 선택 시 2점)
const DEF_AWARD_RULE_CUTOFF_ISO = "2026-02-08";
const isoOnOrAfter = (a: string, b: string) => String(a) >= String(b);
...
const setDefLegacy = (tid: TeamId, pid: string | null) => {
  if (readonly) return;
  patchSession({
    defAwards: {
      ...(cur.defAwards || { A: null, B: null, C: null, D: null } as any),
      [tid]: pid
    }
  });
};

const setDefNew = (tid: TeamId, slot: 0 | 1, pid: string | null) => {
  if (readonly) return;
  const curVal = (cur.defAwards || ({} as any))[tid];
  const arr = getDefAwardArray(curVal);
  const next = [...arr];
  if (!pid) {
    // clear slot
    next.splice(slot, 1);
  ...
  // compact + dedupe while preserving order
  const seen = new Set<string>();
  const compact = next.filter(Boolean).filter(id => {
...
const isNewDefRule = isoOnOrAfter(sessionDate, DEF_AWARD_RULE_CUTOFF_ISO);
  ...
  function calcScores(session: Session, dateKey?: string) {
    ...
    const normName = (v: any) => normKey(v)
      .replace(/\s+/g, " ")
      .replace(/[()\[\]【】{}<>]/g, " ")
      .replace(/(GK|골키퍼|필드)/gi, " ")
      .replace(/\s+/g, " ")
      .trim();

    const resolvePid = (key: string): string | null => {
      const k0 = normKey(key);
      if (!k0) return null;

      // 1) id direct
      if (players.some(p => p.id === k0)) return k0;

      // 2) exact name
      const exact = players.find(p => p.name === k0);
      if (exact) return exact.id;

      // 3) normalized name match (handles "홍길동(GK)" 같은 꼬리표)
      const nk0 = normName(k0);
      const byNorm = players.find(p => normName(p.name) === nk0);
      if (byNorm) return byNorm.id;

      // 4) common delimiters (예: "홍길동 | 기타")
      const parts = k0.split(/[|,;/]/).map(s => normKey(s)).filter(Boolean);
      for (const part of parts) {
        if (players.some(p => p.id === part)) return part;
        const ex = players.find(p => p.name === part);
        if (ex) return ex.id;
        const nn = normName(part);
        const bn = players.find(p => normName(p.name) === nn);
        if (bn) return bn.id;
      }
      return null;
    };

    const safePid = (v: any) => resolvePid(String(v ?? ""));

    const sessionISO = ensureSunday(dateKey || sessionDate);
    const isNewRuleForThisSession = isoOnOrAfter(sessionISO, DEF_AWARD_RULE_CUTOFF_ISO);

    const defAwardIdsForTeam = (team: TeamId): string[] => {
      const v: any = (session.defAwards as any)?.[team];
      if (Array.isArray(v)) return v.map((x: any) => safePid(x)).filter(Boolean) as string[];
      if (typeof v === "string" && v) { const id = safePid(v); return id ? [id] : []; }
      return [];
    };

    const defPoints = (team: TeamId | "-", pid: string): number => {
      if (team === "-") return 0;
      const ids = defAwardIdsForTeam(team);
      if (!isNewRuleForThisSession) {
        // 과거(컷오프 이전): 팀당 1명 2점
        const one = ids[0] || null;
        return one === pid ? 2 : 0;
      }
      // 2026-02-08부터: 2명 선택 시 각 1점, 1명만 선택 시 2점
      if (ids.length === 1) return ids[0] === pid ? 2 : 0;
      return ids.includes(pid) ? 1 : 0;
    };

    const standings = computeStandings(session.matches);
    const activeTeams = getActiveTeamsSafe(session as any);
    const teamBonusByTeam = computeTeamBonus(standings.filter(r => activeTeams.includes(r.team)), (session as any)?.hasTeamD || false);
    ...
    asArray(session.matches, []).forEach(m => {
      const hg = asNumber(m.hg, 0), ag = asNumber(m.ag, 0);
      if (hg > ag && m.gkHome) { const gid = safePid(m.gkHome); if (gid) gkWins[gid] = (gkWins[gid] || 0) + 1; }
      if (ag > hg && m.gkAway) { const gid = safePid(m.gkAway); if (gid) gkWins[gid] = (gkWins[gid] || 0) + 1; }
    });
    ...
    asArray(session.matches, []).forEach(m => {
      ...
      // ✅ 득점/도움이 특정 날짜에서 전체 순위 누적에 빠지는 케이스 방지:
      // - ms 키가 pid가 아닌 이름/꼬리표/가공된 값으로 저장된 경우에도,
      //   "해당 경기 팀 로스터에 포함된 선수 pid"를 기준으로 역매핑해서 점수를 복원한다.
      const involved = new Set<string>([
        ...asArray(session.rosters[m.home], []),
        ...asArray(session.rosters[m.away], []),
      ]);

      const getStatForPid = (pid: string): any | null => {
        if (!pid) return null;
        const pname = players.find(p => p.id === pid)?.name || "";
        const candidates = [pid, pname, normName(pname)].map(x => normKey(x)).filter(Boolean);
        ...
        // pid가 뒤에 붙는 형태 (예: "abc123|필드", "abc123 (field)")
        for (const [k, v] of Object.entries(ms)) {
          const kk = normKey(k);
          if (!kk) continue;
          if (kk === pid || kk.startsWith(pid) || kk.includes(pid)) return v as any;
        }

        // 마지막 안전장치: resolvePid로 돌려서 매칭
        for (const [k, v] of Object.entries(ms)) {
          if (resolvePid(k) === pid) return v as any;
        }
        return null;
      };

      // 1) 우선 로스터 기반 pid에 대해 점수 복원/누적
      involved.forEach(pid => {
        const s = getStatForPid(pid);
        if (!s) return;
      ...
      // 2) 로스터에 없지만 기록에 존재하는 키(예: 임시 선수/과거 포맷)도 보존
      Object.entries(ms).forEach(([pidKey, s]) => {
        const pid = resolvePid(pidKey);
        if (!pid) return;
        if (involved.has(pid)) return;
        const base = out[pid] || { goals: 0, assists: 0, cleansheets: 0 };
        out[pid] = {
          goals: base.goals + asNumber((s as any).goals, 0),
          assists: base.assists + asNumber((s as any).assists, 0),
      ...
      if (asNumber(m.ag, 0) === 0 && m.gkHome) {
        const gid = safePid(m.gkHome);
        if (gid) {
          const b = out[gid] || { goals: 0, assists: 0, cleansheets: 0 };
          out[gid] = { ...b, cleansheets: (b.cleansheets || 0) + 1 };
        }
      }
      if (asNumber(m.hg, 0) === 0 && m.gkAway) {
        const gid = safePid(m.gkAway);
        if (gid) {
          const b = out[gid] || { goals: 0, assists: 0, cleansheets: 0 };
          out[gid] = { ...b, cleansheets: (b.cleansheets || 0) + 1 };
        }
    ...
    Object.keys(out).forEach(pid => {
      ...
      return eff === "GK";
            ...
            const def = defPoints(team as any, pid);
  ...
  const dailyScores = useMemo(() => calcScores(cur, sessionDate), [cur, players, globalTeamNames, sessionDate]);
                ...
                return (
                  <div key={tid} className="team-card">
                    ...
                    <div className="row" style={{ marginTop: 8 }}>
                      ...
                      {/* 2026-02-08부터: 팀당 최대 2명(각 1점), 1명만 선택 시 2점 / 그 이전: 팀당 1명(2점) */}
                      {!isNewDefRule ? (
                        <select
                          value={(() => {
                            const v = (cur.defAwards as any)?.[tid];
                            return (typeof v === "string" ? v : "") as any;
                          })()}
                          onChange={e => setDefLegacy(tid, e.target.value || null)}
                          disabled={readonly}
                        >
                        ...
                        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                          {([0, 1] as const).map((slot) => {
                            const arr = getDefAwardArray((cur.defAwards as any)?.[tid]);
                            const val = (arr[slot] || "") as any;
                            return (
                              <select
                                key={slot}
                                value={val}
                                onChange={e => setDefNew(tid, slot, e.target.value || null)}
                                disabled={readonly}
                              >
                                <option value="">{slot === 0 ? "(1번 선택)" : "(2번 선택)"}</option>
                                {asArray(cur.rosters[tid], []).map(pid => {
                                  const name = players.find(p => p.id === pid)?.name || pid;
                                  return <option key={pid} value={pid}>{name}</option>;
                                })}
